...
func NewJanusObject(url string, clientid uint64, ptype int, Qps int) *JanusData {
	...
	// qps bps
	res.sendTextLen = 802
	res.sendTime = 1000 / (Qps * 1000 / 8 / 800)
	...
	//fmt.Println(data)
	//fmt.Println("++++++++ send time =%d len=%d n=%d ", res.sendTime, res.sendTextLen, n-1)
	res.sendData = data
...
func (obj *JanusData) SendDataChannelData() {
	go func() {
		...
		for {
			...
			total += 1
			time.Sleep(time.Duration(obj.sendTime) * time.Millisecond)
		}
	}()
}
